...
import { CoordsToAddrResponse, InstantSearchResponse, PointToPointResponse, SubwayMappingDataResponse } from './NaverMapTypes'
...
export class NaverMapCrawler {
    ...
    static async coordsToAddr (coords: string) { // coords는 xxx.xxxxxxx,yy.yyyyyyy 형식의 string (소수점 7자리)
        const response = await fetch(`https://map.naver.com/v5/api/geocode?request=coordsToaddr&version=1.0&sourcecrs=${this.crs}&output=json&orders=addr,roadaddr&coords=${coords}`)
    ...
    static async pointToPointByURL (url: string, departureTime: Date) {
        ...
        if (paths[0] !== 'v5' || paths[1] !== 'directions' || paths.length < 4)
            throw new Error('Unknown URL')
        const params = await Promise.all(paths.slice(2, 4).map(async (raw) => { // start, end
            const data = raw.split(',') // 순서대로 좌표1, 좌표2(epsg:3857), 이름, 번호, 종류
            const coords = proj4('EPSG:3857').inverse([parseFloat(data[0]), parseFloat(data[1])]).map((v) => v.toFixed(7).toString()).join(',')
            const name = data[2]
            const type = data[4]
            if (type === 'BUS_STATION') {
